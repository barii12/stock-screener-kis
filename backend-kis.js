...
async function analyzeMarketContextAuto() {
  console.log('📊 시장 자동 분석 중...');
  const KOSPI_STOCKS = ['005930', '000660', '373220', '207940', '005380'];
  const KOSDAQ_STOCKS = ['036570', '251270', '259960', '352820', '328130'];
  
  let kospiTotal = 0, kosdaqTotal = 0, kospiCount = 0, kosdaqCount = 0;
  
  for (const code of KOSPI_STOCKS) {
    try { const data = await getKisStockPrice(code); kospiTotal += data.changePercent; kospiCount++; } catch(e) {}
  }
  for (const code of KOSDAQ_STOCKS) {
    try { const data = await getKisStockPrice(code); kosdaqTotal += data.changePercent; kosdaqCount++; } catch(e) {}
  }
  
  return {
    kospiChange: kospiCount > 0 ? parseFloat((kospiTotal / kospiCount).toFixed(2)) : 0,
    kosdaqChange: kosdaqCount > 0 ? parseFloat((kosdaqTotal / kosdaqCount).toFixed(2)) : 0,
    leadingSector: 'IT',
    marketStrength: 'neutral'
  };
}
...
async function scanAndRecommendStocks(marketContext) {
  console.log('🎯 추천 종목 스캔 중...');
  const TOP_STOCKS = ['005930', '000660', '373220', '207940', '005380', '000270', '068270', '035720', '036570', '251270', '259960', '352820', '323410', '377300', '328130', '196170'];
  const results = [];
  
  for (let i = 0; i < TOP_STOCKS.length; i++) {
    try {
      const stockData = await getKisStockPrice(TOP_STOCKS[i]);
      const chartData = await getKisChartData(TOP_STOCKS[i]);
    ...
    if (i < TOP_STOCKS.length - 1) await new Promise(r => setTimeout(r, 100));
  }
...
// ============================================================
  ...
  ws.on('message', async (message) => {
    try {
      ...
      else if (data.type === 'recommendStocks') {
