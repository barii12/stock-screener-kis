...
async function searchKisStock(query) {
  try {
    ...
    // KIS는 주식 검색이 직접적이지 않음
    // 대신 네이버 API로 코드 조회 후 KIS로 데이터 가져오기
    const response = await axios.get(
...
// ═══════════════════════════════════════════════════════════
// 4. KIS API - 일봉 차트 데이터 (기술 지표용)
// ═══════════════════════════════════════════════════════════
async function getKisChartData(stockCode, period = '100') {
  try {
    const token = await getKisAuthToken();
    if (!token) throw new Error('토큰 발급 실패');
...
function validateTechnicals(stockData, chartData) {
  ...
  if (ma5 > ma10 && ma10 > ma20 && ma20 > ma50 && ma50 > 0) {
    ...
    result.score += 25;
    result.details.push('✓ 완벽 정배열 (5MA>10MA>20MA>50MA)');
    result.strength = 'strong';
  } else if (ma5 > ma10 && ma10 > ma20) {
    result.score += 15;
    result.details.push('✓ 부분 정배열 (5MA>10MA>20MA)');
    result.strength = 'normal';
  } else {
    result.details.push('✗ 역배열 상태');
    return result;
  }
  ...
  // 52주 신고가 대비
  const high52 = stockData.high52Week;
  if (high52 > 0) {
...
function analyzeSectorRotation(stockData, marketContext) {
  ...
  // 실제 구현: KIS API에서 업종 코드로 조회
  const sector = stockData.sector || '기타';

  if (leadingSectors.some(s => sector.includes(s.trim()))) {
    result.isLeadingSector = true;
    result.sectorScore = 30;
    result.details.push(`✓ 주도섹터 진입 (${sector})`);
  } else {
    result.sectorScore = 10;
    result.details.push(`⚠ 중립섹터 (${sector})`);
  }
...
function calculateRSRating(stockData, marketContext) {
  ...
  if (rsMultiple > 3) {
    result.rsRating = 30;
    result.relativeStrength = 'strong';
...
wss.on('connection', (ws) => {
  console.log('✅ 클라이언트 연결됨 (한국투자증권 OPEN API)');
  clients.push(ws);
      ...
      if (data.type === 'checkStock') {
        ...
        if (!stockCode) {
          ws.send(JSON.stringify({ 
            type: 'error', 
            message: `'${input}' 종목을 찾을 수 없습니다. 정확한 종목명이나 코드를 입력해주세요.`
          }));
          return;
        }
        ...
        if (!stockData) {
          ws.send(JSON.stringify({ 
            type: 'error', 
            message: `한국투자증권 API에서 '${input}' 데이터를 조회할 수 없습니다.`
          }));
          return;
        }
...
app.get('/api/health', (req, res) => {
  res.json({ 
    status: 'OK', 
    port: PORT, 
    mode: '한국투자증권 OPEN API',
    requiresAuth: true,
    apiStatus: KIS_API_KEY !== 'YOUR_API_KEY' ? 'Configured' : 'Not Configured'
  });
});
...
server.listen(PORT, () => {
  ...
  ║                                                    ║
  ║   ✅ API 설정됨: ${KIS_API_KEY === 'YOUR_API_KEY' ? '❌ 필요' : '✓ 완료'}     ║
  ╚════════════════════════════════════════════════════╝
  `);
});
